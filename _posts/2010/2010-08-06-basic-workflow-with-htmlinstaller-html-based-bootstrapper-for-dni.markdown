...
Let's look at the HTML source in this bootstrapper and see how we can make changes to it. There are two documents embedded with htmlInstaller by default: _index.html_ and _style.css_. We'll ignore the stylesheet and focus on the index.html page, examining it top-to-bottom.
...
This is a standard HTML page with an _html_ opening tag, a _head_ and a _body_. The stylesheet is imported with a custom HTML tag, `@import` – HTMLayout-specific tags are documented [here](http://web.archive.org/web/20100724054323/http://www.terrainformatica.com/htmlayout/tags.whtm).

There are a few built-in widgets defined by their _id_'s. All such widgets are optional and have well-known behavior within the bootstrapper.
